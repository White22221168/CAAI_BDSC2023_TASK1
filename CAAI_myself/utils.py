...
def ccorr(a, b):
    ...
    # print(b.shape)
    if a.shape[1]%2!=0:
        a = th.cat((a,th.zeros(a.shape[0],1).cuda()),dim=1)
        b = th.cat((b,th.zeros(b.shape[0],1).cuda()),dim=1)
        re = th.fft.irfftn(th.conj(th.fft.rfftn(a, (-1))) * th.fft.rfftn(b, (-1)), (-1))[:,:a.shape[1]-1]
    else:
        re = th.fft.irfftn(th.conj(th.fft.rfftn(a, (-1))) * th.fft.rfftn(b, (-1)), (-1))
    return re

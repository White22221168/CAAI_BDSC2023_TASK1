...
from model2 import CompGCN_DistMult,ConvE
...
def main(args):
    ...
    # compgcn_model = DataParallel(compgcn_model)
    compgcn_model = compgcn_model.to(device)
    checkpoint = th.load('checkpoint4/baseline_ckpt.pth')
    compgcn_model.load_state_dict(checkpoint)
    ...
    for epoch in range(args.max_epochs):
        ...
        for step, batch in enumerate(data_iter['train']):
            ...
            pred	= compgcn_model(graph,sub, rel)
            # tr_loss = loss_fn(logits, label)
            vec = th.gather(pred, 1, obj.reshape(-1, 1))
            vec = vec.reshape(-1) 
            # pred = pred[:,0]
            label = (1.0 - 0.1)*label + (1.0/graph.num_nodes())
            tr_loss	= loss_fn(vec, label)
            train_loss.append(tr_loss.item())
            # train_loss1.append(tr_loss)
            optimizer.zero_grad()
            tr_loss.backward()
            optimizer.step()
            # # 每 batch_size 次更新一次梯度
            # if (step + 1) % batch_size == 0:
            #     train_loss1 = np.sum(train_loss1)
            #     train_loss1.backward()
            #     optimizer.step()
            #     optimizer.zero_grad()
            #     train_loss1 = []
        # 处理最后一个batch，如果batch_size不能整除训练数据总量
        # if len(train_loss1) > 0:
        #     train_loss1 = np.sum(train_loss1)
        #     train_loss1.backward()
        #     optimizer.step()
        #     optimizer.zero_grad()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", dest="output_dir", default="./output4", help="Output dir", )
    parser.add_argument("--ckpt_dir", dest="ckpt_dir", default="./checkpoint4", help="Checkpoint dir", )

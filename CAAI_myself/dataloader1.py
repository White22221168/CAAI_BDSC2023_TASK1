...
class TrainDataset(Dataset):
    def __init__(self, triples, sr2o, num_ent, num_neg=1):
        ...
        for (s, r), o_list in self.sr2o.items():
            self.triples.append([s, r])
        self.num_ent = num_ent
        self.num_neg = num_neg
        print("new_triples:{}".format(len(self.triples)))
    ...
    def __getitem__(self, idx):
        ...
        src, rel = ele
        triples = [torch.LongTensor([src, rel])]
        # trp_label = self.get_label(self.sr2o[(src, rel)])
        # trp_label = (1.0 - 0.1)*trp_label + (1.0/self.num_ent)
        ...
        #         break
        return torch.stack(triples, 0), torch.tensor(list(self.sr2o[(src, rel)]))

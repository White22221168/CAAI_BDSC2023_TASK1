...
class ConvE(nn.Module):
    def __init__(self, 
        ...
        self.conv = nn.Conv2d(1, 16, (3, 3), bias=True)
    ...
    def forward(self,graph, sub, rel):
        ...
        e1_emb = n_feats[sub, :].view(-1, 1, self.dim1, self.dim2)#el_emb; batch*1*20*10
        nn.init.xavier_normal_(e1_emb)
        rel_emb = r_feats[rel, :].view(-1, 1 ,self.dim1, self.dim2)
        nn.init.xavier_normal_(rel_emb)
...
class ConvE1(nn.Module):
    ...
    def forward(self, sub, rel,graph):

...
class TrainDataset(Dataset):
    def __init__(self, triples, sr2o, num_ent, weight,num_neg=1):
    ...
    def __getitem__(self, idx):
        ...
        # while True:
        #     # neg_obj = random.randint(0, self.num_ent - 1)
        #     userid = np.arange(self.num_ent)
        #     neg_obj = np.random.choice(userid, p=self.weight)
        #     if neg_obj not in self.sr2o[(src, rel)]:
        #         triples.append(torch.LongTensor([src, rel, neg_obj]))
        #         labels.append(torch.FloatTensor([0.0]))
        #     if len(triples) > self.num_neg:
...
class Data(object):
    def __init__(self, data_dir, num_workers, batch_size):
        ...
        #负采样权重计算
        self.weight = pd.read_csv("negsample_weight.csv")["count"]
        a = 0.1/299889
        preds = [1.0/299889]*299889
        preds = preds+a*self.weight
        self.preds = preds/preds.sum()
        ...
        with open("./triples.pickle","wb") as f:
            pkl.dump(self.data["train"],f)
        with open("./sr2o.pickle","wb") as f:
            pkl.dump(self.sr2o["train"],f)
        
        def get_train_data_loader(split, batch_size, shuffle=True):
            return DataLoader(
                TrainDataset(
                    self.data[split], self.sr2o[split], self.num_ent,self.preds
                ),
